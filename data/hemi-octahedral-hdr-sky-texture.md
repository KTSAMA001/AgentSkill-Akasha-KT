...
- 四条正方形边统一对应地平线，便于用 footprint 驱动的窄范围重采样处理边缘。物理近地雾属于天空传输模型，不能用固定角宽或固定颜色雾带替代显示接缝滤波。
...
{
    // 本纹理只保存上半球；下半球由地面/空气底图处理。
    // 边缘连续性使用源纹素与屏幕像素 footprint，不能硬编码宽雾带。
    n.y = max(n.y, 0.0);
...
天空方向
  ...
  → 按源纹素/屏幕像素 footprint 做上半球窄接缝滤波
  → 与共享空气传输得到的下半球地面/近地介质底图衔接
  → Exposure
...
| 地平线四边有缝或宽亮带 | Clamp、方向映射、源纹素 footprint、物理近地介质 | 检查四象限和五个基准方向；先用 atlas 对比球面预览区分“烘焙内容过亮”和“显示滤波扩宽”。显示滤波应随源纹素/屏幕像素角宽变化，物理介质则必须同时参与消光与入射散射，二者禁止合并成固定 2°～5° 雾带 |
...
- [WebGL2 体积云的统一积分后端：从实时预览到渐进收敛与离线烘焙](./webgl2-unified-volumetric-cloud-rendering.md) - 说明 Hemi-Oct 渐进方向缓存、统一后端和“物理浅层介质 / 显示 footprint 接缝”分层。
...
- [2026-08-11] 地平线合同纠错：旧文把“四边统一落在地平线”概括为便于窄雾带处理，容易把物理近地介质与 Hemi-Oct 显示接缝混为一层。WebGL2 固定快照确认固定 2.4°～5° 滤波会把指定 128² 近地平线纹素估算值约 0.74° 扩成宽亮带；当前代码已改为由源纹素角和屏幕像素角共同决定 footprint，物理近地介质继续在共享传输中同时贡献消光与入射散射。八面体纹素角宽随方向变化，且用户侧地平线观感仍待完整验收，因此这里只把分层原则和源码修改记为已确认，不宣称视觉问题已经闭环。
