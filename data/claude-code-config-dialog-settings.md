...
**状态**：✅ 已验证
**可信度**：⭐⭐⭐⭐
**适用版本**：Claude Code v2.1.25+；2.1.117+ `/config` 持久化行为变化；2.1.176 设置/配置层校验
...
| **Fast mode** | 开关 | `false` | 快速模式。`v2.1.154+` 默认使用 Opus 4.8，`v2.1.142 ~ v2.1.153` 默认 Opus 4.7；仅适用于 Anthropic API / 订阅 + usage credits，不适用于 Bedrock / Vertex / Foundry / Claude Platform on AWS |
...
- [Claude Code Changelog](https://code.claude.com/docs/en/changelog) - 2.1.117+ `/config` 持久化变化、2.1.176 最新版本
...
- [Claude Code 官方设置文档](https://code.claude.com/docs/en/settings) — 说明配置作用域、两个配置文件的职责划分与优先级
- [Claude Code 官方认证文档](https://code.claude.com/docs/en/authentication) — 说明认证方式、凭证优先级与 CLI/Desktop 差异
- [Claude Code Fast Mode](https://code.claude.com/docs/en/fast-mode) — 说明默认模型、适用范围与 usage credits 约束
...
- [2026-06-14] 时效性复核：基于官方 Settings / Authentication / Fast Mode 文档更新到 2.1.176 口径，确认作用域优先级仍为 managed > CLI 参数 > local > project > user，并修正 Fast Mode 默认模型与适用平台说明。
