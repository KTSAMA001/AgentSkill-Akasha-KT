...
**状态**：✅ 已验证
**可信度**：⭐⭐⭐⭐（官方文档验证）
**适用版本**：Claude Code v2.1.142（官方 latest，2026-05-14）；本机 `claude --version` 为 2.1.133（2026-05-15 验证）

### 概要
Claude Code 是 Anthropic 推出的 AI 编程助手，支持终端 CLI、桌面应用、Web、VS Code 和 JetBrains IDE 五大入口。2.1.x 后期重点增强了后台代理、Agent View、Routines、Remote Control、插件分发、Windows PowerShell 支持、Hooks/MCP/Skill 生态，以及 1M 上下文和 Opus 4.7 `xhigh` effort。
...
| 优势 | 说明 |
|------|------|
| **1M 上下文窗口** | Opus 4.7 / 4.6 系列支持长上下文，适合大型代码库 |
...
| 模型 | 状态 | 特点 |
|------|------|------|
| **Claude Opus 4.7** | GA | 当前最强编码模型，支持 `xhigh` effort；Max/Team Premium 默认可用 |
| **Claude Opus 4.6** | GA | 旗舰编码模型，1M 上下文；可通过环境变量保留 Fast Mode 使用 |
...
Fast Mode（`/fast`）：2.1.142 起默认使用 Opus 4.7；如需临时固定到 Opus 4.6，可设置 `CLAUDE_CODE_OPUS_4_6_FAST_MODE_OVERRIDE=1`。
