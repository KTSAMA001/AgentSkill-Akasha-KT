# 历史方案：Unity 输出型 AssetPostprocessor 的强恢复重导设计
...
**更新日期**：2026-07-24
**状态**：❌ 已废弃
**可信度**：⭐⭐⭐⭐（机制经 Unity 2022.3 LTS 实践验证；默认架构结论已废弃）
**适用版本**：Unity 2021.2+；实践验证版本为 Unity 2022.3 LTS

### 概要

本记录保留一套曾经完整实现并验证过的“自动证明与恢复漏处理”强保证方案，包括逐资源结果、未完成任务、Import Worker 临时回执、Domain Reload 审计和记录损坏恢复。相关 Unity 生命周期与跨进程机制事实仍可参考，但把这些能力写成输出型 `AssetPostprocessor` 的默认稳定架构属于过度概括，已被后续实践废弃。一般工具应优先采用语义配置依赖、旧/新范围差分、范围进出移动处理和人工强制全量重导，见新的最小策略记录。

> ⚠️ **废弃原因**：旧结论重复监控了 Unity 已负责的普通资源、`.meta` 和 Importer 变化，并为低频、可由人工全量重导恢复的异常建立了长期状态协议。机制能够运行不代表收益足以覆盖迁移、损坏恢复、跨进程回执、状态提交和回归测试成本。只有业务明确要求自动证明“没有漏处理”，且已有可复现故障证明 Unity 原生能力不足时，才应从本记录选择强恢复组件。
...
| 资源或处理记录不一致 | 强恢复产品要求下才使用导入回执、完整性检查和未完成任务恢复 | 把该机制作为所有导入工具的默认要求 |

代码、配置、资源状态的生命周期不同。配置语义与旧/新范围差分仍是必要结论；但普通资源状态应先交给 Unity Asset Pipeline。只有强恢复要求成立时，才需要额外保存处理结果并补偿代码或配置不可用期间的遗漏。
...
`AssetDatabase.GetAssetDependencyHash` 可作为资源、meta、目标平台和 Importer 版本变化的聚合信号，但不能单独证明处理器已成功执行，也不能把它的每次变化都直接解释为源资源变化。旧方案继续核对源文件 SHA-256、独立导入设置指纹和处理器级回执，是为了满足强恢复证明；一般工具不应因此重复监控 Unity 已负责的普通资源变化。

#### 七、可选强恢复层：逐资源处理结果

以下结构只适用于产品明确要求自动证明并恢复漏处理的场景，不是默认推荐。若采用，可按资源和处理器记录实际结果：
...
#### 八、可选强恢复层：两阶段提交与跨进程回执

当产品要求跨中断自动恢复，并且 Unity 提供的同步导入边界与导入日志不足以满足结果证明时，批量重导不能只把“调用了 `ImportAsset`”当作完成，可使用持久化未完成任务：
...
若强恢复方案需要把处理器级自定义结果从 Import Worker 传给主 Editor，由于两者不是同一进程，不能依赖静态内存共享回执。此时才需要把“本次导入的临时回执”和“可跨重启恢复的长期处理记录”分成两层：
...
#### 十一、强恢复记录文件必须验证结构，而不只是 JSON 可解析
...
下表记录旧强恢复方案内部的实现风险，只应在已经决定采用强恢复层之后阅读；不得据此反推一般工具必须实现右侧全部机制。
...
- [Unity 输出型 AssetPostprocessor 的最小重导策略](./unity-assetpostprocessor-minimal-reimport-strategy.md) - 当前默认方案；先依赖 Unity 原生导入，只补充外部配置依赖、范围差分和人工兜底。
- [Unity 编辑器资源导入工具架构（配置驱动 + AssetPostprocessor 自动化）](./unity-asset-import-tool-architecture.md) - 配置驱动、文件夹规则和编辑器入口的基础架构。
...
- [2026-07-24] 重大修正。后续项目复盘确认，本记录把一套可运行的强恢复实现过度概括成了输出型后处理器的默认稳定架构。逐资源状态、源/依赖哈希审计、未完成任务、Worker 临时回执、Domain Reload 完整性检查和记录损坏恢复的收益不足以覆盖长期复杂度，且重复了 Unity Asset Pipeline 已承担的普通资源变化检测。记录改为废弃的历史方案；保留已验证生命周期事实和强保证场景下的实现参考，默认决策迁移到最小重导策略。
