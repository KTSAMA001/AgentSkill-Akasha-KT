...
**更新日期**：2026-07-30
**状态**：✅ 已验证
**可信度**：⭐⭐⭐⭐（球壳云、LogRGB、导入、运行时采样和历史 512/1024 多视角烘焙已有实践证据；当前源码的 32/16/8 动态短瓦片已通过低负载连续性检查，但尚未用当前源码重新完成 512/1024 高质量验收；空白工程完全重写也未独立验证）
...
| 当前源码已验范围 | 32/16/8 动态短瓦片的低负载连续性、非空和有限值检查 | 证明链路能工作，不等于当前源码已经通过高分辨率画质门禁 |
| 历史高分辨率证据 | 固定 128 瓦片阶段完成过 512/1024 烘焙与多视角检查 | 可支持球壳云、HDR、编码和采样思路；不能直接证明当前动态瓦片实现的 512/1024 结果 |
...
ScriptableObject 烘焙参数
    ↓ 归一化、单位和输出目录检查
离线烘焙 Material
    ↓ 当前工具按工作量选择 32×32 / 16×16 / 8×8 动态短瓦片执行体积积分
RGBAHalf 线性 HDR 瓦片
    ↓ 按整图像素位置写回
完整半八面体 RGBAHalf 母图
    ├─ 可选：导出 EXR 母版
...
7. 使用下方参数快照和当前工具至少完成 128 冒烟并得到非空云层；若要声称当前源码达到正式画质，还必须完成 512 多视角验收。历史 512/1024 结果可以帮助选参数，但不能替代这一步。
...
| 当前 32/16/8 动态瓦片源码 | `48² / 8` 视线步 / `2` 光照步 / `1 spp` 的跨瓦片检查；四个象限均非空，未发现非有限值 | 先完成 128 冒烟，再逐级做 512；1024 放在 512 通过之后 |
| 2048 与更高步数 | 尚无本次实践证据 | 需要单变量升级、耗时、设备稳定性、输出 hash 和多视角截图 |

因此，2048、384 视线步、64 太阳步和 8 子样本都只是更高成本候选。即使历史版本曾完成 1024，当前动态瓦片源码也不能跳过 128/512 回归直接继承“已验证高质量”的结论。
...
2. 从菜单 `KT Labs/静态体积云天空烘焙器` 打开窗口。若还没有参数资产，点击“创建默认参数资产”；也可以从 Create 菜单创建同类型参数资产。
...
7. 选中 PNG 检查 importer 和 `userData` metadata。然后执行菜单 `KT Labs/静态体积云天空/从最新烘焙创建时间戳对照场景`。纯静态结果使用这个时间戳入口；另一个“更新固定验收场景”入口要求附加数据，不属于本文复现流程。
...
这只是保守的 TDR 风险分档，不是总耗时预测，也不保证任何极端参数绝不触发设备重置。当前证据只覆盖一次 48² 低负载跨瓦片检查，尚未覆盖当前动态瓦片源码的 512/1024 高质量烘焙；源码在该检查后若继续变化，还应重新编译并重跑最小受影响门禁。每个瓦片必须使用整图 UV、整图纹素坐标和相同参数；`ReadPixels` 写入整图的指定偏移，全部瓦片完成后统一 `Apply(false, false)`。若瓦片边界可见，优先检查 `_BakeUvScaleOffset`、全局随机种子和最后一行/列的非整瓦片尺寸，不要改云噪声掩盖拼接错误。
...
本 SOP 的 Inspector 顺序、菜单、开关、按钮、输出命名、编码范围、导入设置和运行时解码均已对照当前源码；当前 32/16/8 动态瓦片只完成低负载回归，历史固定瓦片阶段的 512/1024 证据不能拼接成“当前版本已完成高质量验证”。本文允许使用现有模块的人建立独立静态烘焙闭环，也给出了从零实现时不可缺失的组件、公式、顺序和验收门禁。

仍需明确：当前没有在一份全新的空白 Unity 工程中，仅凭本文重新手写全部样板代码并做第二次独立验收。因此已验证的是核心链路和已留证的具体实现快照；“当前源码的 512/1024 画质已经通过”以及“任意 Unity 版本的空白工程复制后必然一次编译通过”都不在已验证范围。复现到其他版本时，应把编译差异和改动写入验证记录，而不是扩大本文的适用版本声明。
...
当前 Runner 按单片元嵌套工作量在 32×32、16×16、8×8 中动态选择瓦片，阈值分别为 160000 和 640000；静态 Base 按低太阳角可能达到的 128 遮光步做保守估算。当前源码已完成 48² 低负载跨瓦片回归，但还没有完成 512/1024 的独立高质量验收。固定 128×128 瓦片阶段曾有 512/1024 证据，只能保留为历史参照，不能证明当前动态瓦片代码已经通过同一门禁。
...
| 正式判断 | 512 | 128 | 12 | 2 | 多视角、接缝、色带与异常黑块；当前动态瓦片仍待完成此档 |
...
下面的 `ResolveTileSize` 对应当前动态短瓦片实现。它截至本次记录只通过 48² 低负载跨瓦片检查，不是当前源码 512/1024 已通过的证明。导入设置属于另一条数据契约，应分别验证，不能因为 importer 正确就推导瓦片渲染正确。
