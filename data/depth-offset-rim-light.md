...
在视图空间中，法线的 **XY 分量**恰好指向物体的"外侧"方向。沿法线 XY 偏移一小段距离后，去采样深度图中的实际深度：
...
内部像素：                      边缘像素：
  ...
  depth(偏移) ≈ depth(原始)      depth(偏移) >> depth(原始)
  ↓                              ↓
  不是边缘                       是边缘 ✅
```

- **内部像素**：法线指向的表面附近仍在同一物体上 → 偏移采样点深度 ≈ 当前深度 → 深度差小 → 不是边缘
- **边缘像素**：法线指向外侧 → 偏移采样点落在背景或其他远处物体上 → 深度差大 → 是边缘
