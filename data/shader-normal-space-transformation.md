...
本记录只讲法线变换本身，不重复展开通用线性代数。若对列向量、点积、矩阵尺寸、行乘列、转置、单位矩阵、逆矩阵或 `a · b = a^T b` 的记号不熟悉，请先阅读：

- [面向图形学初学者的线性代数基础](./linear-algebra-basics-for-graphics.md)

理解下面推导只需要记住三条规则：`a · b = a^T b`、`(AB)^T = B^T A^T`、`A^T A^-T = I`。
...
- [面向图形学初学者的线性代数基础](./linear-algebra-basics-for-graphics.md) - 点积、矩阵乘法、转置、逆矩阵与 HLSL `mul` 的前置知识
...
- [2026-07-14] 结构调整：将通用线性代数基础迁移到独立记录，本记录只保留法线专属推导、数值验证和 Unity 实现，并通过相关记录引用前置知识。
