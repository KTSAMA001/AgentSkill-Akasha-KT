...
2026-06-16 后续交互调查进一步确认：GUI 不应再把配置保存层级暴露成“全局配置、项目配置、档案、Provider”等多组近似概念。用户真正需要的是两个共享池和一个项目引用关系：

| 配置对象 | 用户动作 | 保存位置 | 项目如何使用 |
|----------|----------|----------|--------------|
| API 配置 | 添加、选择、修改、删除 API 连接 | 用户级共享配置 | 项目只保存选中的 API 配置 ID |
| 提示词配置 | 添加、选择、修改、删除日志规范 | 用户级共享配置 | 项目只保存选中的提示词配置名 |
| 项目引用 | 为某个仓库选择 API 配置 + 提示词配置并安装 hook | 仓库 `.git/info` 的薄引用 | 提交时加载引用并回到共享池取完整配置 |

因此，“全局默认”只能表示未单独设置项目时使用哪一组默认选择，而不是一个独立配置分类；某个项目下拉框必须能看到同一个共享池里的所有 API 配置和提示词配置。保存过的 API 配置若在项目视图中不可选，通常说明运行时只读取了项目薄引用或旧结构，未把用户级共享池合并进项目编辑上下文。
...
- API 配置和提示词配置是两类弹窗，不应在主界面塞入完整编辑表单；主界面只做“使用哪个配置”的绑定动作。
- 新建配置时才输入名称；保存修改时不重复出现名称输入框，避免用户误以为每次保存都会新建或重命名。
- “添加项目”只应出现在项目列表区域；当前项目工具栏不应再放重复添加按钮，否则用户无法判断哪个入口负责项目池、哪个入口负责 hook。
- “检查当前项目”如果只刷新 hook 状态，应合并为状态刷新或自动刷新，不应作为高优先级主按钮长期占位。
- 项目列表不能把“共享配置池”伪装成项目卡片；未选项目时可以管理默认选择，但 hook 安装、卸载、项目状态检查必须明确不可用。
- 项目卡片需要稳定的选中反馈、可读状态标识和不被滚动条遮挡的文字；列表滚动区不应影响右侧表单控件的滚轮操作。
- 当窗口宽度不足时，按钮应保持可见并在布局内自然换位或滚动，而不是被挤出右侧；这类问题应通过布局策略和滚动容器处理，不靠固定像素凑位置。
- GUI 文案要以用户动作命名，例如“管理 API 配置”“使用提示词配置”“保存项目选择”，少用 `Provider`、`Profile` 这类内部词。

UI 文案也要按用户任务命名，而不是按内部实现命名。例如“请求协议”“编辑 API 配置”“保存修改”“使用提示词配置”比 `Provider`、`Profile`、`Base URL + API Key` 更容易理解。

#### 2026-06-16 修改时间线

以下时间来自本地源码文件修改时间，用于回溯这一轮设计和实现脉络；记录中只保留相对文件名，不写入本机绝对路径或真实业务仓库路径。

| 时间 | 范围 | 主要内容 |
|------|------|----------|
| 2026-06-16 16:06 | `examples/global.config.json`、`examples/project.config.json` | 更新示例配置，体现共享 API 配置池、提示词配置池和项目薄引用的分层。 |
| 2026-06-16 16:40 | `package_source_desktop.ps1` | 调整源码包生成逻辑，保留运行时虚拟环境、排除缓存和临时文件，避免交付包结构误导用户。 |
| 2026-06-16 17:39 | `tests/test_gui_offscreen.py` | 增加 GUI 离屏回归，覆盖项目重新打开后仍可选择共享 API 配置和提示词配置，以及项目文件只保存引用。 |
| 2026-06-16 19:06 | `git_commit_ai/cli.py`、`git_commit_ai/qt_gui.py` | 统一运行时配置合成、共享池加载、稳定 API 配置 ID、GUI 管理弹窗和项目选择保存逻辑。 |
| 2026-06-16 19:07 | `tests/test_core.py`、`README.md` | 补充核心测试和用户文档，说明 API 配置池、提示词配置池、项目引用、脱敏保存与 README 中的目录结构边界。 |

这一轮最关键的交互结论是：配置管理要从“按项目保存一套完整配置”改成“全局共享池 + 项目选择引用”。主界面只表达当前项目正在使用哪组 API 和哪套提示词；复杂编辑都进入专门弹窗。这样用户修改一次 API 配置后，所有使用该配置的项目都能同步生效，也不会出现“全局保存了但项目里选不到”的错觉或实际缺陷。
...
18. “全局默认”是默认选择，不是配置池分类；项目视图必须复用同一个 API 配置池和提示词配置池。
19. API 配置和提示词配置要允许任意搭配：同一 API 可服务多套提交规范，同一提交规范也可切换不同 API。
20. GUI 不应把调查、校验或脱敏审查过程写进用户文档或正式记录正文；正式记录只保留可复用的设计结论、验证结果和必要边界。
...
- [2026-06-16] 修正：补充共享配置池交互模型、项目引用边界和本轮文件修改时间线。写入前已执行 `git pull origin main`、全库结构校验和重复检测；语义脱敏审查确认本次只记录相对文件名、通用测试仓库描述、占位符 URL/Key/ID 与可复用交互结论，不写入真实本机路径、内部项目名、API Key、完整提交上下文或业务资产标识。
