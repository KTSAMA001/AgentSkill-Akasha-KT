# Claude Code 2.1 功能全览 (2.1.176)

**标签**：#claude-code #reference #tools #ai
**来源**：官方文档 + CLI `--help` 输出 + Changelog + 社区实践
**收录日期**：2026-03-25
**来源日期**：2026-06-12（v2.1.176 发布日）
**更新日期**：2026-06-25
**状态**：✅ 已验证
**可信度**：⭐⭐⭐⭐⭐ (官方文档 + CLI 实测)
**适用版本**：官方 latest 2.1.176；本机验证 2.1.111

### 概要
Claude Code 2.1.x 完整功能清单，更新到官方 2.1.176。涵盖 15 大能力域：交互模式、内置工具、多代理、会话管理、非交互/SDK 模式、Git 集成、MCP、Hooks、Plugin、Skill、IDE 集成、Memory、Chrome 集成、结构化输出、CLI 标志与环境变量。

### 2.1.81 -> 2.1.176 版本差异摘要

| 领域 | 主要新增/变化 |
|------|---------------|
| 会话/代理 | `claude agents` Agent View、`/branch` 当前会话分支、`/fork <directive>` 后台子代理、`/background`、后台 session 参数分发、远程会话和 mobile push 增强 |
...
| 模型 | Anthropic API 主线切到 Opus 4.8 / Sonnet 4.6，`best/fable` alias 生效；Fast Mode 在 v2.1.154+ 默认 Opus 4.8 |
...
| 模型覆盖 | 可指定 fable / sonnet / opus / haiku；Anthropic API 当前 `opus` 主线为 4.8，`sonnet` 为 4.6 |
...
| 当前会话分支 | `/branch [name]` | 在当前点创建会话副本并切入，原会话可用 `/resume` 返回 |
| 恢复时分叉 | `--resume` / `--continue` + `--fork-session` | 恢复历史或最近会话时创建新的 session ID |
| Fork 子代理 | `/fork <directive>` | 后台子代理继承完整会话并执行旁路任务，完成后回传结果 |
...
| `--resume [value]` / `-r` | 恢复会话 |
| `--continue` / `-c` | 继续最近会话 |
| `--fork-session` | 恢复时创建新 session ID，需配合 `--resume` 或 `--continue` |
...
- [Claude Code 最新功能 (2026-06)](./claude-code-latest-features-2026.md) - 2.1.176 更新记录
...
- [Claude Code 后端模型接入](./claude-code-backend-models.md) - 多模型 LLM Gateway
...
- [2026-06-14] 时效性复核：官方 changelog 确认 latest 2.1.176（2026-06-12），本机 `claude --version` 为 2.1.111；补充 Opus 4.8 / Sonnet 4.6 / Fable 5 alias 与 Fast Mode 默认模型切换的新基线。
