...
需要注意：这组数据可以证明“最终表现与 SRP Batcher 被大量打断高度一致”，但不能仅凭现象就直接断言 Unity 在该材质动画录制路径内部必然使用了 `MaterialPropertyBlock`；如果需要对 Unity 内部实现下结论，应另外结合官方资料或更针对性的取证手段。
...
- [2026-05-29] 补充实测：记录一次批量对象下的对照实验。对照组为直接录制材质参数，实验组为代理字段 + 控制器中转。结果显示控制器模式的主要收益来自 SRP Batcher 路径保持稳定，而不是脚本 CPU 自身显著更低。数据已脱敏，仅保留对象规模、帧率、Profiler 样本和工程结论。
