...
6. 🔴 **CRITICAL - 格式化写入**：按对应类型的写入规范写入 `data/` 目录
7. 🟡 **IMPORTANT - 保存资源**：如有图片等资源，按资源文件规范保存至 `assets/` 并使用相对路径引用
8. 🟡 **IMPORTANT - 打标签**：确保至少 1 个领域标签 + 1 个类型标签
9. 🔴 **CRITICAL - 更新索引**：更新 [INDEX.md](../INDEX.md) 的文件清单（如有新标签则同步更新 tag-registry.md（如新标签））
10. 🔴 **CRITICAL - 提交推送**：Git Commit & Push
