...
| [claude-code-bypass-permissions.md](../data/claude-code-bypass-permissions.md) | #claude-code #tools #experience | ✅ 已验证 | 通过全局 `settings.json` 配置 `permissions.defaultMode` 为 `bypassPermissions`，使 Claude Code 每次启动默认跳过所有权限检查，无需每次加 `--dangerously-skip-permissions` 参数。 |
